<?php
/**
 * Deprecated same-origin upload proxy.
...
if (!$wp_load) {
    ...
    echo json_encode(['success' => false, 'error' => 'WordPress could not be loaded for the StitchSense upload proxy.']);
    exit;
}
...
if (!is_array($payload) || (empty($payload['file_data_uri']) && empty($payload['file_url']))) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'No valid StitchSense pattern file payload was received.']);
    exit;
}

$result = StitchSense_Workflow_Client::post_form('upload', $payload, 300);
