...
- [ ] Add Expo unit/component tests for session restoration, API errors, entitlement gates, pattern binding, and destructive confirmations. *(2026-09-12: Expo now has a Node-backed unit-test script plus pure coverage for session restoration/token rotation, entitlement gates, user-facing API error mapping, pattern chat/rewrite binding selectors, and destructive account/data deletion confirmation tokens and warning copy. Component-level flows and broader destructive confirmations remain.)*
