...
- [ ] Add structured logs, request correlation IDs, crash reporting, uptime checks, latency/error metrics, and alert routing. *(2026-09-08: mobile API requests now send `x-request-id`, multipart uploads include client-surface/request correlation headers, and user-facing operational API errors can include a support code. External uptime checks, crash reporting, dashboards, and alert routing remain.)*
- [ ] Create dashboards and alerts for API availability, authentication failures, workflow failures, webhook failures, database capacity, and storage errors. *(2026-09-08: scheduled GitHub Actions uptime checks now probe public `/health` and deep `/ready` every 15 minutes; dedicated dashboards and external alert routing remain.)*
